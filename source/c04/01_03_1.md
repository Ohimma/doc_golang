...
JSON(JavaScript Object Notation, JS 对象简谱) 是一种轻量级的数据交换格式。JS语言中一切都是对象，任何数据类型都可以通过json表示   
...
序列化是指 将key-value结构体的数据类型(比如结构体/map/切片)序列化成json字符串
...
反序列化是指 将json字符串 反序列化成 key-value结构体的数据类型(比如结构体/map/切片)
