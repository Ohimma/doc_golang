...
* Go 中，字符本质是一个整数，直接输出时是utf-8对应的编码码值(%d)
* Go 中，给某个变量赋一个数字，格式化输出，会输出数字对应的unicode字符
* 字符类型是可以进行计算的
...
func main() {
    ...
    //要给字符合适的类型，否则会溢出
    var c3 byte = '北'   // int8 会溢出
    var c3 int = '北'    // int32 没问题
...
* 字符串一旦赋值，就不可修改了
* Go中使用了utf-8，极大的避免了中文乱码问题，并且节省了空间   
